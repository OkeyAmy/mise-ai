/**
 * Audio Worklet Processor for Gemini Live Integration
 * 
...
class AudioProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.bufferSize = 4096; // Standard buffer size for audio processing
   ...
   * @param {Float32Array[][]} outputs - Array of output channels (unused to avoid feedback)
  ...
  process(inputs, outputs, parameters) {
    ...
    // Note: We don't output audio to avoid feedback loop
    ...
    for (let i = 0; i < inputChannel.length; i++) {
      ...
      if (this.bufferIndex >= this.bufferSize) {
        ...
        // Send the PCM data to the main thread for transmission to Gemini
...
// Register the audio processor with the Web Audio API
